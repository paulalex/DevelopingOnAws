package uk.co.developingonaws.eventdrivenlamda;
...
public class Calculator implements RequestHandler<S3Event, String> {
    ...
    public String handleRequest(S3Event event, Context context) {
        ...
        if (numbers.length > 0)
        {
            ...
            double count = Arrays.stream(numbers).count();
            double average = Arrays.stream(numbers).average().orElse(Double.NaN);
            result = String.format("Min: %s Max: %s Average: %s count: %s", min, max, average, count);
        }
